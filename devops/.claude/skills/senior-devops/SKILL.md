...
**After deployment completes:** Launch a `deployment-verifier` agent with the environment, service name, deployed version, and health-check URL. Provide baseline error rate and p99 latency if available. The agent returns a GREEN / DEGRADED / CRITICAL verdict with the specific Paperclip action to take.

If the verdict is DEGRADED or CRITICAL, immediately launch an `incident-assessor` agent with the deployment context and the verifier's signal summary to get a rollback vs fix-forward recommendation before taking any action.
...
**After rollback completes:** Launch a `deployment-verifier` agent again to confirm the rollback restored healthy signal before notifying the Tech Lead of success.

## Resources

- `references/cicd_pipeline_guide.md` — CI/CD patterns, best practices, anti-patterns
